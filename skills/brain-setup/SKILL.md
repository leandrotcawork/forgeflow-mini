...
- **section** (optional): Jump directly to a specific config section. If omitted, show the section menu.
  - Valid sections: `database`, `hooks`, `linters`, `resilience`, `subagents`, `learning`, `context_loading`, `token_budgets`, `token_optimization`, `consolidation`, `lesson_escalation`, `weight_decay`
...
| 2 | hooks | 3 | Hook profile, profiles map, individual overrides |
| 3 | linters | 5 | File extension to linter command mapping |
| 4 | resilience | 2 | Circuit breaker and strategy rotation settings |
| 5 | subagents | 5 | Subagent dispatch, models, and fallback |
| 6 | learning | 5 | Confidence scoring and lesson promotion |
| 7 | context_loading | 7 | Token budgets per tier and always-loaded items |
| 8 | token_budgets | 7 | Per-agent token in/out limits |
| 9 | token_optimization | 2 | Compact threshold and context pressure levels |
| 10 | consolidation | 3 | Trigger mode, auto-propose, approval required |
| 11 | lesson_escalation | 2 | Escalation threshold and action |
| 12 | weight_decay | 4 | Decay rate, min weight, max stale days |
...
1. Read the template config from the plugin's `templates/brain/brain.config.json`
